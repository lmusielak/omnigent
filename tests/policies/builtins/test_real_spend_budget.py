...
REAL business spend reported by an external HTTP spend oracle's ``/quota``
endpoint. That endpoint reports spend as ``spend_today`` / ``spend_mtd``
sub-objects (each ``{"total_usd": <USD>, ...}``) and the operator-set
thresholds under ``spend_thresholds`` keyed by horizon (``daily`` / ``mtd``,
each ``{"ask_usd", "limit_usd"}``). The tests author payloads in a convenient
FLAT spec and nest them to that real shape via :func:`_nest` at the transport
boundary, so gating intent stays readable while the wire contract exercised
is the oracle's true nested one. Layers:

- **Layer 1** — direct callable: the fail-open matrix (every oracle
  failure mode must ALLOW, non-negotiable), positive ASK /
...
- **Layer 3** — the approval-state key is distinct from ``cost_budget``'s:
  approving one guard's ASK must never suppress the other's.
"""
...
# A "healthy" quota spec with headroom on both horizons — the ALLOW baseline
# every gating test perturbs. Authored FLAT for readability; _nest() renders
# it to the oracle's real nested /quota shape before it hits the transport.
...
def _nest(flat: dict[str, Any]) -> dict[str, Any]:
    """Render a flat quota spec into the oracle's real nested /quota shape.

    Tolerant by construction: a field is emitted only when its flat key is
    present, so a test that deletes a flat key (or sets it to a bad value)
    produces a nested response missing (or malforming) exactly that field —
    which is what the fail-open matrix asserts against.

    :param flat: A (possibly partial / perturbed) copy of :data:`_QUOTA_OK`.
    :returns: The nested ``/quota``-shaped payload.
    """
    nested: dict[str, Any] = {}
    if "spend_today" in flat:
        nested["spend_today"] = {"total_usd": flat["spend_today"], "partial": False}
    if "spend_mtd" in flat:
        nested["spend_mtd"] = {"total_usd": flat["spend_mtd"], "partial": False}
    thresholds: dict[str, Any] = {}
    daily: dict[str, Any] = {}
    if "daily_ask" in flat:
        daily["ask_usd"] = flat["daily_ask"]
    if "daily_limit" in flat:
        daily["limit_usd"] = flat["daily_limit"]
    if daily:
        thresholds["daily"] = daily
    mtd: dict[str, Any] = {}
    if "mtd_ask" in flat:
        mtd["ask_usd"] = flat["mtd_ask"]
    if "mtd_limit" in flat:
        mtd["limit_usd"] = flat["mtd_limit"]
    if mtd:
        thresholds["mtd"] = mtd
    nested["spend_thresholds"] = thresholds
    return nested
...
def _json_transport(payload: Any, status_code: int = 200) -> httpx.MockTransport:
    ...
    :param payload: JSON-serializable body (already in wire shape).
...
def _oracle_transport(flat: dict[str, Any], status_code: int = 200) -> httpx.MockTransport:
    """Build a transport serving a flat quota spec as the real nested shape.

    :param flat: A (possibly perturbed) copy of :data:`_QUOTA_OK`.
    :param status_code: HTTP status code to respond with.
    :returns: A configured :class:`httpx.MockTransport`.
    """
    return _json_transport(_nest(flat), status_code=status_code)
...
def test_allows_on_non_200_response() -> None:
    ...
    policy = real_spend_budget(transport=_oracle_transport(_QUOTA_OK, status_code=500))
    assert policy(_tool()) == {"result": "ALLOW"}
...
def test_allows_on_missing_key(missing_key: str) -> None:
    """Any missing required field must fail OPEN (ALLOW)."""
    payload = dict(_QUOTA_OK)
    del payload[missing_key]
    policy = real_spend_budget(transport=_oracle_transport(payload))
...
def test_allows_on_oracle_error_stub() -> None:
    """The oracle's own top-level ``{"error": ...}`` stub response fails OPEN."""
...
def test_allows_on_error_stub_even_with_other_keys_present() -> None:
    """A top-level ``error`` key takes precedence over otherwise-valid data.

    A stub response might carry partial/placeholder fields alongside
    ``error`` — the gate must still fail open rather than trust them.
    """
    payload = _nest(_QUOTA_OK)
    payload["error"] = "quota service not configured"
    policy = real_spend_budget(transport=_json_transport(payload))
    assert policy(_tool()) == {"result": "ALLOW"}


# ── nested-shape malformation (regression guard for the oracle contract) ──
# These express failures the flat spec cannot: they exercise the exact
# boundary where the engine consumes the oracle's real nested shape. The
# first is the direct regression test for the contract-drift bug where the
# oracle emitted spend as nested {total_usd} sub-objects but the gate read a
# bare top-level float.


def test_allows_when_spend_today_is_a_bare_number() -> None:
    """``spend_today`` as a bare float (not a {total_usd} object) fails OPEN.

    Regression guard: the oracle reports spend as nested sub-objects. A gate
    that expected a bare number here would read a dict where it wanted a
    float and must fail open, never gate on the un-read figure.
    """
    payload = _nest(_QUOTA_OK)
    payload["spend_today"] = 1.0  # the OLD, wrong flat shape
    policy = real_spend_budget(transport=_json_transport(payload))
    assert policy(_tool()) == {"result": "ALLOW"}


def test_allows_when_spend_today_total_is_null() -> None:
    """A partial/unavailable source (``total_usd: null``) fails OPEN."""
    payload = _nest(_QUOTA_OK)
    payload["spend_today"] = {"total_usd": None, "partial": True}
    policy = real_spend_budget(transport=_json_transport(payload))
    assert policy(_tool()) == {"result": "ALLOW"}


def test_allows_when_spend_today_is_error_stub() -> None:
    """A spend sub-object that is its own ``{"error": ...}`` stub fails OPEN."""
    payload = _nest(_QUOTA_OK)
    payload["spend_today"] = {"error": "spend today unavailable: boom"}
    policy = real_spend_budget(transport=_json_transport(payload))
    assert policy(_tool()) == {"result": "ALLOW"}


def test_allows_when_spend_thresholds_not_a_dict() -> None:
    """A non-object ``spend_thresholds`` fails OPEN."""
    payload = _nest(_QUOTA_OK)
    payload["spend_thresholds"] = "unavailable"
    policy = real_spend_budget(transport=_json_transport(payload))
    assert policy(_tool()) == {"result": "ALLOW"}


def test_allows_when_spend_thresholds_missing_a_horizon() -> None:
    """``spend_thresholds`` missing the mtd horizon entirely fails OPEN."""
    payload = _nest(_QUOTA_OK)
    payload["spend_thresholds"].pop("mtd")
    policy = real_spend_budget(transport=_json_transport(payload))
    assert policy(_tool()) == {"result": "ALLOW"}


def test_allows_when_spend_thresholds_is_error_stub() -> None:
    """A ``spend_thresholds`` error stub fails OPEN."""
    payload = _nest(_QUOTA_OK)
    payload["spend_thresholds"] = {"error": "spend thresholds unavailable"}
    policy = real_spend_budget(transport=_json_transport(payload))
    assert policy(_tool()) == {"result": "ALLOW"}


def test_gates_on_full_realistic_oracle_payload() -> None:
    """A full, realistic /quota body (extra spend fields + other keys) gates.

    Fidelity check: the real oracle response carries partial/per-source
    breakdown fields on the spend sub-objects and unrelated top-level keys
    (ppu, backends, lanes, the pre-existing quota-% ``thresholds`` object).
    The gate must ignore all of that and still DENY on an over-limit day.
    """
    payload = {
        "updated_at": "2026-07-13T00:00:00+00:00",
        "ppu": {"anthropic": {"enabled": False}},
        "spend_today": {
            "date": "2026-07-13",
            "total_usd": 12.5,  # over daily_limit 10.0 → DENY
            "partial": False,
            "proxy_usd": 2.5,
            "claude_overage_today_usd": 10.0,
            "claude_overage_mtd_usd": 120.0,
        },
        "spend_mtd": {"total_usd": 40.0, "partial": False},
        "spend_thresholds": {
            "daily": {"ask_usd": 5.0, "limit_usd": 10.0},
            "mtd": {"ask_usd": 300.0, "limit_usd": 500.0},
        },
        # pre-existing quota-% knobs under the SAME name the gate must NOT read
        "thresholds": {"claude_max_pct": 90, "codex_max_pct": 90},
        "backends": {"claude": {"available": True}},
        "lanes": {"claude-opus": {"available": True}},
    }
    policy = real_spend_budget(transport=_json_transport(payload))
    result = policy(_tool())
    assert result["result"] == "DENY"
    assert "12.50" in result["reason"]
    assert "daily" in result["reason"]


# ══════════════════════════════════════════════════════════════════════
# Layer 1 — direct callable: threshold validation edge cases
# ══════════════════════════════════════════════════════════════════════
...
def test_allows_when_both_horizons_have_headroom() -> None:
    ...
    policy = real_spend_budget(transport=_oracle_transport(_QUOTA_OK))
    assert policy(_tool()) == {"result": "ALLOW"}
...
# ══════════════════════════════════════════════════════════════════════
# Layer 3 — approval-state key is distinct from cost_budget's
# ══════════════════════════════════════════════════════════════════════
...
def test_ask_approved_keys_are_distinct_from_cost_budget() -> None:
    ...
    An ASK approval on one guard must never silently suppress the other
    guard's prompts. Distinct key names are the mechanism.
    """
