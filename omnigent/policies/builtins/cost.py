...
# spend oracle HTTP service. The oracle owns both the running spend totals
# (spend_today / spend_mtd) AND the four operator-set thresholds (daily
# ask/limit, MTD ask/limit) — an operator can tighten or loosen the budget by
# editing the oracle's own config, with no redeploy of this policy or the
# agent bundle that references it.
#
...
# horizons before trusting the numbers — a violation is treated exactly like
# any other malformed oracle payload: fail open (ALLOW).
#
# This factory's ASK-approval session_state keys are deliberately distinct
# from cost_budget's ``SESSION_COST_ASK_APPROVED_STATE_KEY``
...
# The spend oracle's quota endpoint. Overridable via factory_params for
# non-default deployments; defaults to a localhost port the oracle listens on.
...
# the request / tool_call phase), and ANY failure to reach the oracle in time
# must fail open rather than stall the turn.
_REAL_SPEND_TIMEOUT_S_DEFAULT = 2.0

# The nested oracle-response paths the gate projects to its internal flat
# figures. The oracle's /quota response carries real spend as sub-objects
# ({"total_usd": <num>, "partial": ..., ...}) and the operator thresholds
# under a "spend_thresholds" object keyed by horizon; any missing or
# non-numeric field fails open (see ``_fetch_real_spend_quota``).
_REAL_SPEND_THRESHOLD_HORIZONS = (
    ("daily", "daily_ask", "daily_limit"),
    ("mtd", "mtd_ask", "mtd_limit"),
)
...
def _fetch_real_spend_quota(
    ...
    """GET the spend oracle and project its nested spend/threshold fields to
    the gate's internal flat figures.

    The oracle's ``/quota`` response carries real spend as sub-objects and
    the operator-set thresholds under a ``spend_thresholds`` object::

        {"spend_today":      {"total_usd": <num>, "partial": <bool>, ...},
         "spend_mtd":        {"total_usd": <num>, "partial": <bool>, ...},
         "spend_thresholds": {"daily": {"ask_usd": <num>, "limit_usd": <num>},
                              "mtd":   {"ask_usd": <num>, "limit_usd": <num>}}}

    A missing / partial-null source is reported as a sub-object without a
    numeric ``total_usd`` (or its own ``{"error": ...}`` stub), which fails
    open — the gate never invents a spend figure it can't read.
    ...
    - the oracle's own top-level ``{"error": ...}`` stub response;
    - a ``spend_today`` / ``spend_mtd`` sub-object that is missing, is an
      ``{"error": ...}`` stub, or whose ``total_usd`` is ``None``, a bool, or
      otherwise non-numeric (a partial / unavailable source);
    - a ``spend_thresholds`` object missing either horizon, or with a
      ``None`` / bool / non-numeric ``ask_usd`` / ``limit_usd``;
    - a threshold pair that fails the strict ``0 < ask < limit`` invariant on
      EITHER horizon — treated as a malformed / degenerate oracle config,
      not a valid non-degenerate one.
    ...
    :returns: A dict with the flat keys ``spend_today``, ``spend_mtd``,
        ``daily_ask``, ``daily_limit``, ``mtd_ask``, ``mtd_limit`` as floats,
        or ``None`` to signal "fail open".
    """
    try:
    ...
    def _is_num(raw: Any) -> bool:
        # bool is an int subclass in Python — exclude explicitly so a stray
        # `true` doesn't get silently coerced to 1.0.
        return raw is not None and not isinstance(raw, bool) and isinstance(raw, (int, float))

    def _spend_total(obj: Any) -> float | None:
        """The numeric ``total_usd`` of a spend sub-object, else ``None``."""
        if not isinstance(obj, dict) or "error" in obj:
            return None
        total = obj.get("total_usd")
        return float(total) if _is_num(total) else None

    spend_today = _spend_total(payload.get("spend_today"))
    spend_mtd = _spend_total(payload.get("spend_mtd"))
    if spend_today is None or spend_mtd is None:
        return None

    thresholds = payload.get("spend_thresholds")
    if not isinstance(thresholds, dict) or "error" in thresholds:
        return None

    values: dict[str, float] = {"spend_today": spend_today, "spend_mtd": spend_mtd}
    for horizon, ask_key, limit_key in _REAL_SPEND_THRESHOLD_HORIZONS:
        rec = thresholds.get(horizon)
        if not isinstance(rec, dict):
            return None
        ask, limit = rec.get("ask_usd"), rec.get("limit_usd")
        if not _is_num(ask) or not _is_num(limit):
            return None
        values[ask_key] = float(ask)
        values[limit_key] = float(limit)
...
def real_spend_budget(
    ...
    *,
    ...
    """Factory: gate on REAL (non-LLM) spend from an external spend oracle.
    ...
    entirely outside the engine by an external spend oracle
    (``GET {oracle_url}``), which reports real spend as ``spend_today`` /
    ``spend_mtd`` sub-objects (each ``{"total_usd": <USD>, ...}``) and the
    operator-set ask/limit thresholds under ``spend_thresholds`` keyed by
    horizon (``daily`` / ``mtd``, each ``{"ask_usd", "limit_usd"}``). See
    :func:`_fetch_real_spend_quota` for the full contract and the fail-open
    matrix.
    ...
    the guard should never block real work because of its own unavailability
    or misconfiguration.
    ...
    :param oracle_url: Full URL of the spend oracle's quota endpoint.
        Defaults to :data:`_REAL_SPEND_ORACLE_URL_DEFAULT`.
...
POLICY_REGISTRY: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "kind": "factory",
        "name": "Real Spend Budget",
        "description": "Gates a session on REAL (non-LLM) business spend reported by an "
        "external spend oracle: GETs the spend_today / spend_mtd sub-objects "
        "(each {total_usd, ...}) and the operator-set spend_thresholds (daily/mtd ask_usd + "
        "limit_usd) from the oracle's quota endpoint. DENYs (the whole turn at the request "
        "phase, or each tool "
        ...
        "params_schema": {
            ...
            "properties": {
                "oracle_url": {
                    "type": "string",
                    "description": "Full URL of the spend oracle's quota "
